...
class QueryResolver {
  getMonster(root, context) {
    return context.monsterStore.getMonsterById(root.id).catch(() => null);
  }
  ...
  searchMonstersByName(root, context) {
    return context.monsterStore.findMonstersByName(root.name).catch(() => []);
  }

  getRandomMonster(root, context) {
    return context.monsterStore.getRandomMonster().catch(() => null);
  }
}
